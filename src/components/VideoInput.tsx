...
//   index: number;
//   onVehicleCount: (count: number, hasEmergencyVehicle: boolean) => void;
// }
...
//       );
//       const emergencyVehicles = predictions.filter(
//         pred => pred.class === 'truck' && pred.score > 0.8 // Assuming emergency vehicles are detected as high-confidence trucks
//       );
...
//         ctx.lineWidth = 2;
//         ctx.strokeStyle = emergencyVehicles.includes(vehicle) ? 'blue' : 'red';
//         ctx.fillStyle = emergencyVehicles.includes(vehicle) ? 'blue' : 'red';
//         ctx.stroke();
//         ctx.fillText(
...
//       onVehicleCount(vehicles.length, emergencyVehicles.length > 0);
//       setDebugInfo(`Detected ${vehicles.length} vehicles, ${emergencyVehicles.length} emergency`);
//     } catch (error) {
...
const VideoInput: React.FC<VideoInputProps> = ({ index, onVehicleCount }) => {
  ...
  useEffect(() => {
    if (inputType === 'upload' && videoRef.current && uploadedMedia) {
      videoRef.current.src = uploadedMedia;
      videoRef.current.load();
    }
  }, [inputType, uploadedMedia]);
